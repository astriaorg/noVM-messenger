...
impl AddressError {
    fn decode(source: bech32::primitives::decode::CheckedHrpstringError) -> Self {
        Self(AddressErrorKind::Decode { source })
    }

    fn invalid_prefix(source: bech32::primitives::hrp::Error) -> Self {
        Self(AddressErrorKind::InvalidPrefix { source })
    }

    fn incorrect_address_length(received: usize) -> Self {
        Self(AddressErrorKind::IncorrectAddressLength { received })
    }
}
...
impl<TFormat, TBytes, TPrefix> AddressBuilder<TFormat, TBytes, TPrefix> {
    ...
    pub fn verification_key(
        self,
        key: &astria_core::crypto::VerificationKey,
    ) -> AddressBuilder<TFormat, WithBytes<'static, NoBytesIter>, TPrefix> {
...
impl TransactionId {
    ...
    pub fn try_from_raw_ref(raw: &raw::TransactionId) -> Result<Self, TransactionIdError> {
        ...
        let inner = <[u8; TRANSACTION_ID_LEN]>::from_hex(&raw.inner)
            .map_err(|err| TransactionIdError(TransactionIdErrorKind::HexDecode { source: err }))?;
        Ok(Self { inner })
    }
}
...
#[cfg(test)]
mod tests {
    use super::{Address, AddressError, AddressErrorKind, Bech32m, ADDRESS_LEN};
    ...
    fn assert_wrong_address_bytes(bad_account: &[u8]) {
        ...
        let AddressError(AddressErrorKind::IncorrectAddressLength { received }) = error else {
