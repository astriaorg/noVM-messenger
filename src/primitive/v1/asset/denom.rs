    ...
    pub fn unchecked_from_parts<I: IntoIterator<Item = (String, String)>>(
        ...
        Self {
            trace: TraceSegments {
                inner: trace
                    .into_iter()
                    .map(|(port, channel)| PortAndChannel { port, channel })
                    .collect(),
            },
            base_denom,
        }
...
impl ParseIbcPrefixedError {
    fn hex(source: hex::FromHexError) -> Self {
        Self(ParseIbcPrefixedErrorKind::Hex { source })
    }
...
mod serde_impl {
    ...
    mod tests {
        ...
        use crate::primitive::v1::asset::{denom::TracePrefixed, Denom};
...
mod tests {
    use super::{
        Denom, IbcPrefixed, ParseIbcPrefixedErrorKind, ParseTracePrefixedErrorKind, TracePrefixed,
    };

    #[test]
    fn parse_ibc_errors() {
        use ParseIbcPrefixedErrorKind::{
            Hex, NotIbcPrefixedPrefixed, TooFewSegments, TooManySegments,
        };
        #[track_caller]
        #[expect(
    ...
    fn parse_trace_errors() {
        use ParseTracePrefixedErrorKind::{
            BaseIsEmpty, ChannelIsEmpty, NotAscii, PortIsEmpty, Whitespace,
        };
        #[track_caller]
        #[expect(
            clippy::needless_pass_by_value,
